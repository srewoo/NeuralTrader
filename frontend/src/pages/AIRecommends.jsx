...
export default function AIRecommends() {
  ...
  const generateRecommendations = async () => {
    ...
    try {
      toast.info("Analyzing top NSE/BSE stocks with AI...\nThis may take 2-3 minutes.");
      ...
      toast.success(
        `Analysis complete! ${summary.total_buy_signals} BUY, ${summary.total_sell_signals} SELL signals found.`,
        { duration: 5000 }
      );
    } catch (err) {
  ...
  const INDIAN_MARKET_SECTORS = [
    "Banking", "IT", "Energy", "Automotive", "Pharma", "FMCG", "Telecom",
    "Metals", "Infrastructure", "Cement", "Financial Services", "Insurance",
    "Chemicals", "Consumer Durables", "Media", "Real Estate", "Healthcare",
    "Power", "Capital Goods", "Retail"
  ];
  ...
  const getSectors = () => {
    ...
    if (recommendedSectors.size > 0) return Array.from(recommendedSectors).sort();
      ...
      <div className="max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="text-3xl font-heading font-bold text-text-primary">AI Picks</h1>
      ...
      <div className="max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="text-3xl font-heading font-bold text-text-primary">AI Picks</h1>
              <p className="text-text-secondary">Generating fresh analysis...</p>
            </div>
          </div>
        </div>
        <Card className="card-surface">
            ...
            <div className="flex flex-col items-center justify-center gap-4">
              ...
              <div className="text-center">
                <p className="text-lg font-medium text-text-primary mb-2">
                  AI is analyzing top NSE/BSE stocks...
                </p>
                <p className="text-sm text-text-secondary">
                  Scanning RSI, MACD, Moving Averages, Bollinger Bands, candlestick patterns & more
                </p>
              </div>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-heading font-bold text-text-primary">AI Picks</h1>
              <p className="text-text-secondary">AI-powered BUY/SELL recommendations for NSE/BSE stocks</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
          ...
          <CardContent className="py-16">
            <div className="text-center">
              ...
              <h3 className="text-lg font-medium text-text-primary mb-2">
                {recommendations?.message || "No Recommendations Yet"}
              </h3>
              <p className="text-text-secondary mb-6 max-w-md mx-auto">
                Click "Generate New" to analyze top NSE/BSE stocks and get AI-powered buy/sell recommendations.
              </p>
              <Button
                onClick={generateRecommendations}
                disabled={generating}
            ...
            <Card className="card-surface border-success/20">
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Badge className="bg-success/20 text-success border-0">{buyRecs.length} stocks</Badge>
                </div>
              </CardHeader>
              <CardContent>
                      ...
                      <AnimatePresence>
                        {buyRecs.map((rec, index) => (
                          <motion.div
                            ...
                            className="p-4 rounded-lg bg-success/5 border border-success/20 hover:border-success/40 transition-all hover:bg-success/10"
                          >
                            <div className="flex items-start justify-between mb-3">
                              <div>
                                <div className="flex items-center gap-2">
                                  <span className="font-data font-bold text-text-primary">{rec.symbol}</span>
                                  <Badge variant="outline" className="text-xs">{rec.sector}</Badge>
                                </div>
                                <p className="text-sm text-text-secondary truncate max-w-[200px]">{rec.name}</p>
                              </div>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              ...
                              {rec.signals?.length > 3 && (
                                <Badge variant="outline" className="text-xs">+{rec.signals.length - 3} more</Badge>
                              )}
                            </div>
            ...
            <Card className="card-surface border-danger/20">
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Badge className="bg-danger/20 text-danger border-0">{sellRecs.length} stocks</Badge>
                </div>
              </CardHeader>
              <CardContent>
                <ScrollArea className="h-[500px] pr-4">
                      ...
                      <AnimatePresence>
                        {sellRecs.map((rec, index) => (
                          <motion.div
                            ...
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ delay: index * 0.03 }}
                            className="p-4 rounded-lg bg-danger/5 border border-danger/20 hover:border-danger/40 transition-all hover:bg-danger/10"
                          >
                            <div className="flex items-start justify-between mb-3">
                              <div>
                                <div className="flex items-center gap-2">
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {rec.signals?.slice(0, 3).map((signal, idx) => (
                                <Badge key={idx} variant="outline" className="text-xs bg-danger/10 border-danger/30 text-danger">
                                  {signal}
                                </Badge>
                              ))}
                              {rec.signals?.length > 3 && (
            ...
            <CardContent className="py-4">
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm text-text-secondary">
                  ...
                  <p>
                    These recommendations are generated based on technical indicators and AI analysis.
                    This is not financial advice. Always conduct your own research and consult with a
